...
let gantt = {
  ...
  selectTaskForDependency: function (e) {
    ...
    if (clicked.element.getAttribute('class') === 'taskDiv' || clicked.element.getAttribute('class') === 'taskDiv active') {
      ...
      if (active !== clicked) {
        // prevent user from setting active task as dependency of its self
        let foundChild = false;
        let foundParent = false;

        function checkChildren(task) {
          // prevents user from setting one of the children of the active tasks as a dependency of the active task.
          for (let i = 0; i < task.childTaskList.length; i++) {
            let child = gantt.findTask(task.childTaskList[i]);
            if (child === clicked) {
              foundChild = true;
          ...
          if (foundChild === true) return false;else return true;
        }

        function checkParents(task) {
          // prevents user from setting one of the parents of the active task as a dependency of the active task.
          for (let i = 0; i < task.parentTaskList.length; i++) {
            let parent = gantt.findTask(task.parentTaskList[i]);
          ...
          if (foundParent === true) return false;else return true;
        }
...
document.getElementById('addDependencyConfirm').addEventListener('click', function (e) {
  gantt.dependencyDecide(e, true);
}, false);
document.getElementById('addDependencyCancel').addEventListener('click', function (e) {
